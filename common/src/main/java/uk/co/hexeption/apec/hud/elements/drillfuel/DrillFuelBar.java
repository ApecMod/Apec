...
public class DrillFuelBar extends Element {
    ...
    public void drawText(GuiGraphics graphics, boolean editMode) {
        ...
        graphics.blit(RenderType::guiTexturedOverlay, drillFuelBarTexture.getResourceLocation(), width, height, 0, 80, 182, 5, drillFuelBarTexture.getWidth(), drillFuelBarTexture.getHeight());

        // Full Bar - with drill fuel color (orange/brown)
        graphics.blit(RenderType::guiTexturedOverlay, drillFuelBarTexture.getResourceLocation(), width, height, 0, 85, (int) (fuelFactor * 182f), 5, drillFuelBarTexture.getWidth(), drillFuelBarTexture.getHeight());
    }
